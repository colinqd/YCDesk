...
function register(safeHandler, logFn, getMainWindow, signalingServer, inputHandler) {
  ...
  signalingServer.onIncomingConnection((data) => {
    ...
    if (w) w.webContents.send('incoming-connection', data)
  })
  signalingServer.onConnectionResult((data) => {
    const w = getMainWindow()
    if (w) w.webContents.send('connection-result', data)
  })
  signalingServer.onOffer((data) => {
    const w = getMainWindow()
    if (w) w.webContents.send('offer', data)
  })
  signalingServer.onAnswer((data) => {
    const w = getMainWindow()
    if (w) w.webContents.send('answer', data)
  })
  signalingServer.onIceCandidate((data) => {
    const w = getMainWindow()
    if (w) w.webContents.send('ice-candidate', data)
  })
}
