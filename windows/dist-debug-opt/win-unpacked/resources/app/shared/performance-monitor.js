class PerformanceMonitor {
  ...
  recordLatency(ms) {
    ...
    if (this._latencySamples.length > 0) {
      ...
      this.metrics.avgLatency = Math.round(sum / this._latencySamples.length)
    }
  }

  recordBandwidth(bytesPerSecond) {
    this.metrics.bandwidth = Math.round(bytesPerSecond / 1024)
  }

  recordPacketLoss(ratio) {
    this.metrics.packetLoss = ratio
  }
  ...
  recordEncodeTime(ms) {
    this.metrics.encoderTime = ms
  }

  recordDecodeTime(ms) {
    this.metrics.decoderTime = ms
  }
