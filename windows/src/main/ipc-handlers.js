...
function init(deviceIdParam, loggerParam) {
  ...
  ipcMain.on('remote-input', (event, inputData) => {
    ...
    if (inputType === 'lock_screen' || inputType === 'unlock_screen') {
      log('info', '[IPC] 收到特殊命令: type=' + type + ', inputType=' + inputType + ', 完整数据: ' + JSON.stringify(inputData))
    } else {
      log('debug', 'DIAG IPC: 收到remote-input, type=' + type + ', inputType=' + inputType + ', x=' + (inputData && inputData.x) + ', y=' + (inputData && inputData.y) + ', button=' + (inputData && inputData.button))
    }
