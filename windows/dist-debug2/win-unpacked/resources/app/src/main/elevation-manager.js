...
class ElevationManager {
  ...
  _getServiceExePath() {
    const possiblePaths = [
      ...
      path.join(process.resourcesPath || '', '../service/YCDeskService.exe'),
      // node-windows daemon wrapper
      path.join(__dirname, '../../service/daemon/YCDeskService.exe'),
      path.join(__dirname, '../../service/node_modules/node-windows/daemon/wrapper.exe')
    ]
  ...
  async installService() {
    ...
    // 优先尝试使用 node-windows 安装脚本
    const installScript = path.join(__dirname, '../../service/install.js')
    try {
      require('fs').accessSync(installScript)
      this.log('使用 node-windows 安装脚本')
      try {
        const output = execSync(`node "${installScript}"`, { encoding: 'utf8', timeout: 30000 })
        this.log(`Install script output: ${output.trim()}`)
        return { success: true, output: output.trim() }
      } catch (e) {
        this.log(`node-windows 安装失败，回退到 sc create: ${e.message}`)
        // 继续使用 sc create 方式
      }
    } catch (e) {
      // install.js 不存在，使用 sc create 方式
    }
