...
class CrashReporter {
  ...
  init() {
    if (!this.enabled) {
      this.logFn.info('[CrashReporter] 崩溃报告已禁用')
      return
    }
  ...
  recordCrash(type, details = {}) {
    ...
    if (this.stats.crashHistory.length > 50) {
      this.stats.crashHistory = this.stats.crashHistory.slice(-30)
    }
  ...
  _cleanupOldLogs() {
    try {
      var files = fs.readdirSync(this.logDir)
        ...
        .map(function(f) { return { name: f, path: path.join(this.logDir, f) } }.bind(this))
        .sort(function(a, b) {
  ...
  clear() {
    ...
    this.stats.lastCrash = null
    this.stats.crashHistory = []
  }
}
