class CanvasObjectPool {
    ...
    if (this.pool.length >= this.maxPoolSize) {
      ...
      for (var j = 0; j < this.pool.length; j++) {
        if (!this.pool[j].inUse) {
          if (!oldestIdle || this.pool[j].createdAt < oldestIdle.createdAt) {
            oldestIdle = this.pool[j]
            oldestIdx = j
          }
        }
      }
  ...
  cleanup() {
    ...
    for (var i = this.pool.length - 1; i >= 0; i--) {
      ...
      if (now - this.pool[i].createdAt > this.maxIdleTime) {
        this.pool.splice(i, 1)
        removed++
      }
    }

    if (removed > 0 && this.logFn) {
      this.logFn('[CanvasObjectPool] 清理了 ' + removed + ' 个空闲对象，池大小: ' + this.pool.length)
    }
  }
  ...
  resize(width, height) {
    for (var i = 0; i < this.pool.length; i++) {
      if (!this.pool[i].inUse) {
        this.pool[i].canvas.width = width
        this.pool[i].canvas.height = height
        this.pool[i].createdAt = Date.now()
      }
    }
  }
  ...
  getStats() {
    return {
      ...
      idleCount: this.pool.length - this.activeCount,
      totalCreated: this.totalCreated,
      totalReused: this.totalReused,
      totalDiscarded: this.totalDiscarded,
      reuseRate: this.totalCreated > 0
        ? (this.totalReused / (this.totalCreated + this.totalReused)).toFixed(2)
        : '0.00',
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = CanvasObjectPool
}