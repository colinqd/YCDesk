/**
 * YCDesk Electron 平台适配层
 * 
 * 提供 Electron 特定的 API 实现
 * 适用于 Windows、Linux、macOS
 * 
 * @module shared/platform/electron-adapter
 */
...
export const platformAdapter = {
    ...
    async getDeviceId() {
        try {
            ...
            const deviceId = secureDeviceId('WIN')
    ...
    get inputHandler() {
        return {
            type: 'robotjs',
            start() {
                log.info('启动 robotjs 输入处理器')
            },
            stop() {
                log.info('停止 robotjs 输入处理器')
            }
        }
    }
}
