...
function validateInputCommand(command) {
  ...
  if (!command.inputType || typeof command.inputType !== 'string') {
    ...
    if (!validTypes.includes(command.inputType)) {
      errors.push(`无效的 inputType: ${command.inputType}, 有效值: ${validTypes.join(', ')}`)
    }
  }
  ...
  if (command.x !== undefined) {
    if (typeof command.x !== 'number') {
      errors.push('x 必须是数字')
    } else if (command.x < 0 || command.x > 1) {
      errors.push('x 必须在 0-1 范围内')
    }
  }
  ...
  if (command.y !== undefined) {
    if (typeof command.y !== 'number') {
      ...
      errors.push('y 必须在 0-1 范围内')
    }
  }
  
  if (command.button !== undefined) {
    if (typeof command.button !== 'number') {
      errors.push('button 必须是数字')
    } else if (command.button < 0 || command.button > 2) {
      errors.push('button 必须在 0-2 范围内')
    }
  }
  
  if (command.deltaY !== undefined && typeof command.deltaY !== 'number') {
    errors.push('deltaY 必须是数字')
  }
  ...
  if (command.keyCode !== undefined && typeof command.keyCode !== 'number') {
    errors.push('keyCode 必须是数字')
  }
  
  if (command.ctrlKey !== undefined && typeof command.ctrlKey !== 'boolean') {
    errors.push('ctrlKey 必须是布尔值')
  }
  
  if (command.shiftKey !== undefined && typeof command.shiftKey !== 'boolean') {
    errors.push('shiftKey 必须是布尔值')
  }
  
  if (command.altKey !== undefined && typeof command.altKey !== 'boolean') {
    errors.push('altKey 必须是布尔值')
  }
  
  if (command.metaKey !== undefined && typeof command.metaKey !== 'boolean') {
    errors.push('metaKey 必须是布尔值')
  }
