...
describe('消息队列', () => {
  ...
  it('入队消息可被取出', () => {
    manager.enqueue({ message: { test: true }, requireAck: false })
    expect(manager.messageQueue.length).toBe(1)
  })

  it('队列满时丢弃最早的消息', () => {
    manager.enqueue({ message: { id: 1 }, requireAck: false })
    manager.enqueue({ message: { id: 2 }, requireAck: false })
    manager.enqueue({ message: { id: 3 }, requireAck: false })
    manager.enqueue({ message: { id: 4 }, requireAck: false }) // 超过 maxQueueSize=3
