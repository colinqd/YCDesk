class MatrixTransformer {
  ...
  applyContainerSize(videoContainer, videoWrapper, screenContainer) {
    // screen-container 尺寸由 CSS 固定
    // video-wrapper 100% 填满 screen-container
    videoWrapper.style.width = '100%'
    videoWrapper.style.height = '100%'
    videoWrapper.style.left = '0px'
    videoWrapper.style.top = '0px'
    
    // video-container 100% 填满 video-wrapper
  ...
  containerToRemote(containerX, containerY) {
    ...
    // containerX, containerY 是相对于变换后容器的内部坐标
    // 当 transform-origin: 0 0 时：
    // - 内部坐标到原始坐标只需要除以 scale
    // - 不需要减去 panX（因为 containerX 已经是内部坐标）
