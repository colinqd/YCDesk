...
class MatrixTransformer {
    ...
    _updateDisplayRect() {
        ...
        const remoteRatio = this.remoteScreenWidth / this.remoteScreenHeight

        // 根据宽高比计算显示区域（保持比例）
        if (remoteRatio > screenRatio) {
            // 远程屏幕更宽，以宽度为基准
            this.displayWidth = this.screenWidth
            this.displayHeight = this.screenWidth / remoteRatio
            this.displayX = 0
            this.displayY = (this.screenHeight - this.displayHeight) / 2
        } else {
            // 远程屏幕更高，以高度为基准
            this.displayHeight = this.screenHeight
            this.displayWidth = this.screenHeight * remoteRatio
            ...
            if (wrapperElement) {
                ...
                if (this.remoteScreenWidth > 0 && this.remoteScreenHeight > 0) {
                    ...
                    if (remoteRatio > containerRatio) {
                        ...
                        wrapperHeight = this.displayWidth / remoteRatio
                        wrapperTop = (this.displayHeight - wrapperHeight) / 2
                    } else {
                        wrapperHeight = this.displayHeight
                        wrapperWidth = this.displayHeight * remoteRatio
