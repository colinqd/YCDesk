class DeviceListUI {
  ...
  _processUpdates() {
    ...
    for (var i = 0; i < batch.length; i++) {
      switch (batch[i].type) {
        case 'full': this._fullRender(fragment, batch[i].devices); break
        case 'add': this._addItem(fragment, batch[i].device); break
        case 'remove': this._removeItem(batch[i].deviceId); break
        case 'update': this._updateItem(batch[i].deviceId, batch[i].updates); break
        case 'clear': this.containerEl.innerHTML = ''; this.deviceCache.clear(); break
      }
    }
  ...
  _updateItem(deviceId, updates) {
    ...
    if (updates.status !== undefined) {
      ...
      if (statusEl) { statusEl.textContent = updates.status; statusEl.className = 'device-status status-' + updates.status }
    }
  }
  ...
  getDeviceCount() { return this.deviceCache.size }
...
if (typeof module !== 'undefined' && module.exports) { module.exports = DeviceListUI }